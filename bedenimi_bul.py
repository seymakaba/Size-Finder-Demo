...
SIZE_TABLE: Dict[str, Dict[str, List[Row]]] = {
    ...
    "male": {
        ...
        "bottom": [
            {"label": "28 / XS", "short": "XS", "fitScore": 1, "numeric": 28, "waist": 84},
            {"label": "30 / S", "short": "S", "fitScore": 2, "numeric": 30, "waist": 86.5},
            {"label": "32 / M", "short": "M", "fitScore": 3, "numeric": 32, "waist": 89},
            {"label": "34 / L", "short": "L", "fitScore": 4, "numeric": 34, "waist": 91.5},
            {"label": "36 / XL", "short": "XL", "fitScore": 5, "numeric": 36, "waist": 94},
            {"label": "38 / 2XL", "short": "2XL", "fitScore": 6, "numeric": 38, "waist": 99},
            {"label": "40 / 3XL", "short": "3XL", "fitScore": 7, "numeric": 40, "waist": 104},
            {"label": "42 / 4XL", "short": "4XL", "fitScore": 8, "numeric": 42, "waist": 109},
            {"label": "44 / 5XL", "short": "5XL", "fitScore": 9, "numeric": 44, "waist": 114.5},
        ],
    },
}
...
BODY_EFFECT = {
    ...
    "male": {
        ...
        "triangle": {"upper": -0.10, "lower": 0.25},
        "oval": {"upper": 0.15, "lower": 0.25},
        "slim": {"upper": -0.25, "lower": -0.25},
    },
}
...
def pick_by_score(gender: str, group: str, score: float) -> Dict[str, Any]:
    ...
    minimum = rows[0]["fitScore"]
    maximum = rows[-1]["fitScore"]
    rounded = clamp(math.floor(score + 0.5), minimum, maximum)

    main = next((row for row in rows if row["fitScore"] == rounded), None)
    if main is None:
        main = rows[0]
        for row in rows:
            if abs(row["fitScore"] - rounded) < abs(main["fitScore"] - rounded):
                main = row

    direction = 1 if score >= rounded else -1
    alt_score = clamp(rounded + direction, minimum, maximum)
    alternative = next((row for row in rows if row["fitScore"] == alt_score), None)
    if alternative is None:
        alternative = next((row for row in rows if row is not main), None) or main

    return {"main": {"row": main}, "alternative": {"row": alternative}, "roundedScore": rounded}
